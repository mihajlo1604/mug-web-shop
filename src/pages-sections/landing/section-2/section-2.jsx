...
export default function Section2() {
  return <FeaturesSection id="features">
      <Container maxWidth="lg">
        ...
        <Grid container spacing={4}>
          {FEATURES_LIST.map((item, index) => <Grid size={{
          xs: 12,
          sm: 6,
          lg: 3
        }} key={item.title}>
            ...
            animationDelay: `${index * 0.1}s`
              ...
              bgcolor: `${item.color}20`
                  ...
                  <Image priority width={32} height={32} alt={`${item.title} icon`} src={`/assets/images/icons/${item.icon}.svg`} />
                </FeatureIcon>
        ...
        background: "linear-gradient(135deg, rgba(25, 118, 210, 0.05) 0%, rgba(156, 39, 176, 0.05) 100%)"
          ...
          <Grid container spacing={3}>
            {["75+ Pre-built Pages", "100% TypeScript", "Advanced Admin Dashboard", "Multi-vendor Support", "Mobile-First Design", "SEO Optimized"].map(benefit => <Grid size={{
            xs: 12,
            sm: 6,
            md: 4
          }} key={benefit}>
