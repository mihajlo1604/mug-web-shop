  ...
  return <Fragment>
      <Wrapper role="article" aria-labelledby={`card-title-${title}`}>
        <Card elevation={1} className="card">
          <Image priority src={imgUrl} width={1175} height={1000} alt={`${title} demo preview screenshot`} sizes="(max-width: 768px) 100vw, (max-width: 1200px) 50vw, 25vw" />
        </Card>

        {status && <StatusChip aria-label={`Status: ${status}`}>{status}</StatusChip>}

        {!disabled && <Link href={previewUrl} target="_blank" rel="noreferrer noopener" aria-label={`Open ${title} demo in new tab`}>
