...
export default function Section4() {
  return <Container maxWidth="md" id="technologies" sx={styles.container}>
      ...
      <StyledContent>
        <Grid container spacing={5}>
          {TECHNOLOGIES.map(item => <Grid size={{
          lg: 3,
          md: 4,
          xs: 6
        }} key={item.title}>
              <Tooltip arrow placement="top" title={`${item.title} - ${item.description}`}>
                <Card elevation={3} className="card">
                  <Image priority width={40} height={40} src={item.imgUrl} alt={`${item.title} logo`} />
