...
export default async function HealthBeautyPageView({
  ...
  const [services, products, navigationList, topNewProducts, mainCarouselData] = await Promise.all([api.getServices(), api.getProducts(selected), api.getNavigation(), api.getTopNewProducts(), api.getMainCarousel()]);
...
// SIDE NAVBAR COMPONENT
  ...
  return <Fragment>
      ...
      <StickyWrapper SideNav={SideNav}>
        {selected ? <Section4 products={products} title={selected} /> : <Fragment>
            {/* BANNER AREA */}
            <Section2 />
