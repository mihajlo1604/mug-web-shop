...
export default async function GiftShopPageView({
  ...
  const [products, navList, carouselData, popularProducts, topSailedProducts] = await Promise.all([api.getAllProducts(selected), api.getCategoryNavigation(), api.getMainCarouselData(), api.getPopularProducts(), api.getTopSailedProducts()]);

  
// SIDE NAVBAR COMPONENT
  const SideNav = <SideNavbar line="dash" navList={navList} sx={{
    borderRadius: 0
  }} />;
  return <Fragment>
      {/* TOP HERO AREA */}
      ...
      <StickyWrapper className="pb-4" SideNav={SideNav}>
        {selected ? <Section6 products={products} title={selected} /> : <Fragment>
            {/* SERVICE LIST AREA */}
            <Section2 />
